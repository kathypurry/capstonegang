...
const PlayerInput = ({ snippet }) => {
  ...
  const {
    states: { currIndex, phase, correctChar, errorChar, startTime, endTime },
  ...
  useEffect(() => {
    let tempCurrIndex = snippet[currIndex] === " " ? currIndex + 1 : currIndex;
    ...
    endIndex = endIndex < 0 ? snippet.length - 1 : endIndex - 1;
    ...
    <div>
      ...
      >
        ...
        <div>
          <input
            style={
              justTyped === correctChar
                ? { backgrounColor: "black", color: "#39ff14" }
                : { backgroundColor: "black", color: "var(--white)" }
            }
            type="snippet"
            ref={inputRef}
            onKeyDown={(e) => {
